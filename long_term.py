...
# --- ĐỊNH NGHĨA CÁC THAM SỐ CHO DỰ ĐOÁN DÀI HẠN ---
STABLE_THRESHOLD_PERCENTAGE = 0.10
PREDICTION_HORIZON = 250 
# ----------------------------------------------------

# --- Data Loading and Preprocessing (Sử dụng phiên bản từ code bạn cung cấp, cập nhật Target) ---
def load_and_preprocess_data(file_path, stable_percentage, horizon, time_steps=10, train_ratio=0.8):
    df = pd.read_csv(file_path, parse_dates=['Date'])
    df = df.sort_values(by='Date').reset_index(drop=True)

    # Kiểm tra nếu df quá ngắn so với horizon + time_steps
    if len(df) < horizon + time_steps + 10: # Thêm một buffer nhỏ
        raise ValueError(f"Dataset quá ngắn ( {len(df)} dòng) cho horizon ({horizon}) và time_steps ({time_steps}) đã chọn.")
    ...
    actual_news_cols = []
    # Sử dụng tên cột 'Sentiment_Score' như trong file code bạn cung cấp
    ...
    elif 'VIX_Close' in df.columns: vix_col_for_change = 'VIX_Close' # Fallback
    
    if vix_col_for_change:
        df[f'{vix_col_for_change}_Change'] = df[vix_col_for_change].diff()
    else: 
        df['VIX_Change_Fallback'] = np.nan
    ...
    # --- CẬP NHẬT ĐỊNH NGHĨA TARGET CHO DÀI HẠN ---
    ...
    if df.empty:
        raise ValueError(f"DataFrame rỗng sau khi dropna Target. Có thể horizon ({horizon}) quá lớn so với dữ liệu.")
    ...
    # Kiểm tra xem có đủ dữ liệu cho cả train và test sau khi trừ time_steps không
    if train_size_index <= time_steps or (len(features_data) - train_size_index) <= time_steps:
        raise ValueError(f"Không đủ dữ liệu để tạo sequence cho train/test. "
                         f"Total samples: {len(features_data)}, train_size_index: {train_size_index}, time_steps: {time_steps}")
    ...
    def create_sequences(features_scaled, original_target_array, time_steps_local):
        ...
        if len(features_scaled) <= time_steps_local: # Cần <= thay vì <
    ...
    # Đảm bảo X_train, X_test có đúng số chiều ngay cả khi rỗng
    if X_train.ndim == 1 and X_train.shape[0] == 0 : X_train = np.empty((0, time_steps, num_actual_features))
    if X_test.ndim == 1 and X_test.shape[0] == 0 : X_test = np.empty((0, time_steps, num_actual_features))
    if X_train.shape[0] == 0 and (X_train.ndim < 3 or (X_train.ndim == 3 and X_train.shape[2] != num_actual_features and num_actual_features > 0) ) : 
        X_train = np.empty((0, time_steps, num_actual_features))
    if X_test.shape[0] == 0 and (X_test.ndim < 3 or (X_test.ndim == 3 and X_test.shape[2] != num_actual_features and num_actual_features > 0) ) : 
...
# --- Định nghĩa không gian tìm kiếm (CÓ THỂ ĐIỀU CHỈNH time_steps CHO DÀI HẠN) ---
space_optimized_single_lstm = [
    Integer(30, 90, name='time_steps'), # Tăng phạm vi time_steps, ví dụ: 30-90 ngày cho dự đoán 1 năm
...
BASE_RESULTS_DIR = f"experiment_results_long_term_h{PREDICTION_HORIZON}_v1" # Thư mục mới
...
def fitness(time_steps, lstm_units1, dropout_rate1, 
    ...
    print(f"Đang thử nghiệm với các tham số (long-term LSTM v1, H={PREDICTION_HORIZON}): {current_params}")
    ...
    model = Sequential(name=f"LongTermLSTM_FitnessCall_{fitness_call_count}")
...
def save_bayesian_opt_summary(results_dir, skopt_result, param_space_used):
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write(f"--- Bayesian Optimization Summary (Long-Term LSTM V1, Horizon={PREDICTION_HORIZON}) ---\n")
    ...
    try:
        ...
        plt.title(f"Convergence Plot (Long-Term LSTM, H={PREDICTION_HORIZON})")
        plt.savefig(os.path.join(results_dir, f"bayesian_opt_convergence_long_term_h{PREDICTION_HORIZON}.png"))
        plt.close()
        ...
        if plot_dims:
            ...
            plt.suptitle(f"Objective Function Landscape (Long-Term LSTM, H={PREDICTION_HORIZON} - Partial)")
            plt.tight_layout(rect=[0, 0, 1, 0.96])
            plt.savefig(os.path.join(results_dir, f"bayesian_opt_objective_landscape_long_term_h{PREDICTION_HORIZON}.png"))
            plt.close()
...
def save_final_model_evaluation(results_dir, metrics, filename_suffix=""):
    ...
    with open(metrics_path, 'w', encoding='utf-8') as f:
        f.write(f"--- Final Model Evaluation Metrics (Long-Term LSTM, H={PREDICTION_HORIZON}{filename_suffix}) ---\n")
        # ... (phần còn lại giống bản trước) ...
...
def main_with_bayesian_optimization():
    ...
    current_run_results_dir = os.path.join(BASE_RESULTS_DIR, f"run_long_term_h{PREDICTION_HORIZON}_v1_{timestamp}") 
    os.makedirs(current_run_results_dir)
    print(f"Kết quả sẽ được lưu vào: {current_run_results_dir}")

    print(f"Bắt đầu Bayesian Optimization (Long-Term LSTM V1, Horizon={PREDICTION_HORIZON})...")
    # Cảnh báo về features cho dài hạn
    print("\nCẢNH BÁO QUAN TRỌNG CHO DỰ ĐOÁN DÀI HẠN:")
    print("Mô hình hiện tại sử dụng các feature hàng ngày và trễ 1 ngày. "
          "Đối với dự đoán dài hạn (ví dụ: 1 năm), các feature này có thể không mang lại nhiều tín hiệu dự báo mạnh mẽ. "
          "Dự đoán dài hạn thường hiệu quả hơn với DỮ LIỆU CƠ BẢN của công ty và các XU HƯỚNG KINH TẾ VĨ MÔ DÀI HẠN hơn. "
          "Kết quả từ thử nghiệm này nên được xem xét cẩn thận trong bối cảnh đó.\n")


    skopt_result = gp_minimize(
        func=fitness, dimensions=space_optimized_single_lstm, 
        n_calls=20, # Giảm số lần gọi cho thử nghiệm dài hạn vì mỗi lần có thể lâu hơn
        n_initial_points=5, 
        random_state=42,
        verbose=True 
    )

    print(f"\n--- Kết quả Bayesian Optimization (Long-Term LSTM V1, Horizon={PREDICTION_HORIZON}) ---")
    ...
    print(f"\n--- Huấn luyện mô hình cuối cùng (Long-Term LSTM V1, Horizon={PREDICTION_HORIZON}) ---")
    ...
    try:
        ...
        if y_train.shape[0] > 0:
            ...
            if len(unique_classes) >= 2: 
                ...
                # Điều chỉnh thủ công nếu cần (ví dụ tăng weight cho lớp ít bị dự đoán đúng)
                # if class_weights_final_model is not None:
                #     if 0 in class_weights_final_model: class_weights_final_model[0] *= 1.2 
                #     if 1 in class_weights_final_model: class_weights_final_model[1] *= 1.1 
                #     if 2 in class_weights_final_model: class_weights_final_model[2] *= 0.9 
    ...
    final_model = Sequential(name=f"LongTerm_LSTM_Final_H{PREDICTION_HORIZON}")
    ...
    summary_file_path = os.path.join(current_run_results_dir, f"final_model_summary_long_term_h{PREDICTION_HORIZON}.txt")
    ...
    history = final_model.fit( 
        X_train, y_train, epochs=100, # Có thể cần ít epochs hơn nếu dữ liệu ít
    ...
    print(f"\nĐánh giá Mô hình Cuối Cùng (Long-Term, H={PREDICTION_HORIZON}, average='{avg_method}'):")
    for name, value in metrics_dict.items(): 
        if isinstance(value, float): print(f"{name}: {value:.4f}")
        else: print(f"{name}: {value}")
    save_final_model_evaluation(current_run_results_dir, metrics_dict, filename_suffix=f"_long_term_h{PREDICTION_HORIZON}")
    
    plot_training_history_custom(history, current_run_results_dir, call_info=f'final_model_long_term_h{PREDICTION_HORIZON}')
    class_names = ['Decrease', 'Stable', 'Increase']
    plot_confusion_matrix_custom(y_test, y_pred_final, current_run_results_dir, class_names=class_names, call_info=f'final_model_long_term_h{PREDICTION_HORIZON}')
    
    model_save_path = os.path.join(current_run_results_dir, f'lstm_long_term_h{PREDICTION_HORIZON}_model.keras')
    final_model.save(model_save_path)
    print(f"\nMô hình cuối cùng đã lưu vào: {model_save_path}")
    
    # Dự đoán cho 'horizon' ngày tới
    ...
    if len(final_features_data) < best_time_steps:
        ...
        if last_data_points_unscaled.shape[1] != final_scaler.n_features_in_:
            ...
            last_actual_date = df_full_processed['Date'].iloc[-1] 
            prediction_summary_path = os.path.join(current_run_results_dir, f"next_horizon_prediction_h{PREDICTION_HORIZON}.txt")
            prediction_text = (f"Dự đoán xu hướng cho {PREDICTION_HORIZON} ngày giao dịch tới (kể từ sau {last_actual_date.strftime('%Y-%m-%d')} - long_term): {next_trend}\n"
            ...
            print(f"Dự đoán cho dài hạn tiếp theo đã được lưu vào: {prediction_summary_path}")
